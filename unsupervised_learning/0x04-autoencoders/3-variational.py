...
def autoencoder(input_dims, hidden_layer_sizes, latent_dims):
    ...
    def normal_sample(inputs):
        ...
        std_norm = keras.backend.random_normal(keras.backend.shape(mean))
        sample = mean + keras.backend.exp(log_stddev) * std_norm
        return sample
    ...
    def VAE_loss(inputs, outputs):
        """ Custom loss function including a a KL divergence term. """
